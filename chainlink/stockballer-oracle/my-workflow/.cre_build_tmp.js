...
var athleteSchema = exports_external.object({
  tokenId: exports_external.number(),
  apiFootballId: exports_external.number(),
...
var configSchema = exports_external.object({
  ...
  apiFootballBaseUrl: exports_external.string(),
  vaultAddress: exports_external.string(),
  chainSelectorName: exports_external.string(),
  gasLimit: exports_external.string(),
  athletes: exports_external.array(athleteSchema),
  priceFormula: exports_external.object({
    ipoPrice: exports_external.number(),
    minPrice: exports_external.number(),
    maxPrice: exports_external.number(),
    yieldPerGoal: exports_external.number(),
    yieldPerAssist: exports_external.number(),
    yieldPerMatch: exports_external.number(),
    penaltyYellowCard: exports_external.number(),
    penaltyRedCard: exports_external.number(),
    penaltyMissedPenalty: exports_external.number()
  })
});
...
function getFormMultiplier(rating) {
  if (rating <= 0)
    return 1000n;
...
function calculatePrice(goals, assists, appearances, rating, yellowCards, redCards, penaltiesMissed, age, formula) {
...
var onCronTrigger = (runtime2, _payload) => {
  runtime2.log("\uD83C\uDFC8 StockBaller Price Oracle - Starting price update");
  ...
  const mockStats = {
    276: { goals: 8, assists: 5, appearances: 20, rating: 7.2, yellowCards: 2, redCards: 0, penaltiesMissed: 1 },
    874: { goals: 25, assists: 8, appearances: 28, rating: 8.5, yellowCards: 3, redCards: 0, penaltiesMissed: 0 },
    1100: { goals: 12, assists: 10, appearances: 25, rating: 8, yellowCards: 1, redCards: 0, penaltiesMissed: 0 },
    154: { goals: 15, assists: 12, appearances: 30, rating: 8.2, yellowCards: 4, redCards: 0, penaltiesMissed: 0 },
    306: { goals: 10, assists: 4, appearances: 22, rating: 7, yellowCards: 5, redCards: 1, penaltiesMissed: 2 }
  };
  const priceUpdates = [];
  for (const athlete of config.athletes) {
    runtime2.log(`\uD83D\uDCCA Processing: ${athlete.name} (ID: ${athlete.apiFootballId})`);
    try {
      ...
      const newPrice = calculatePrice(stats.goals, stats.assists, stats.appearances, stats.rating, stats.yellowCards, stats.redCards, stats.penaltiesMissed, athlete.age, config.priceFormula);
      const priceUsd = Number(newPrice) / 1e6;
      runtime2.log(`\uD83D\uDCB0 ${athlete.name}: $${priceUsd.toFixed(2)}`);
      const callData = encodeFunctionData({
        abi: ProSpectVault,
        functionName: "updatePriceFromOracle",
        args: [BigInt(athlete.tokenId), newPrice]
      });
      const tx = evmClient.sendTransaction(runtime2, {
        call: encodeCallMsg({
      ...
      runtime2.log(`✅ Updated ${athlete.name} price, tx: ${result.txHash}`);
      priceUpdates.push({
        name: athlete.name,
      ...
      runtime2.log(`❌ Error processing ${athlete.name}: ${error}`);
    }
  }
  runtime2.log(`\uD83C\uDFAF Price updates completed: ${priceUpdates.length}/${config.athletes.length} athletes`);
